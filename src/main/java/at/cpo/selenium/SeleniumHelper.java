...
//      options.setHeadless(Boolean.getBoolean("headless"));
	...
	public boolean exists(String locatorDelegate, boolean secret) {
		...
		if (webEl == null) {
			nodeLogPass("exists (" + xpath + ", " + secret + ") - false");
			return false;
		}
		nodeLogPass("exists (" + xpath + ", " + secret + ") - true");
	...
	public void input(String locatorDelegate, String value, boolean secret) {
		...
		try {
			...
			if (descParts.length == 2) {
				...
				try {
					...
					try {
						...
						nodeLogFail(node.addScreenCaptureFromPath(ExtentHelper.screenshotFile(driver)) + "input ("
			...
			if ("ListBox".equalsIgnoreCase(className) || "RadioGroup".equals(className)) {
				...
				nodeLogPass("input (" + xpathDescription + ", '" + value + ")'");
				...
				webEl.clear();
				webEl.sendKeys(value);
				nodeLogPass("input (" + xpathDescription + ", '" + getSecretString(value, secret) + ")'");
				try {
