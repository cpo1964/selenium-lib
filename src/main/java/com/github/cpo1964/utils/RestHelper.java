...
public class RestHelper {
	...
	static {
		if (httpclient == null) {
			try {
				// client without proxy and no defined CookieStore uses BasicCookieStore
	...
	public static CloseableHttpClient buildHttpClient(final HttpHost proxy, final CookieStore store, boolean useSSL)
		...
		HttpClientBuilder httpCB = HttpClientBuilder.create().disableRedirectHandling()
	 ...
	 */
	public static CloseableHttpResponse doHttpPost(String uri, String content, List<NameValuePair> headers)
			throws IOException {
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static CloseableHttpResponse doHttpPost(String uri, ContentType contentType, String content,
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static CloseableHttpResponse doHttpPost(CloseableHttpClient client, String uri, ContentType contentType,
			String content, List<NameValuePair> headers) throws IOException {
		HttpPost httpMethod = new HttpPost(uri);
		if (contentType == null) {
			httpMethod.setEntity(
					new ByteArrayEntity(content.getBytes(StandardCharsets.UTF_8), ContentType.APPLICATION_JSON));
		} else {
	...
	/**
	 * Overload von doHttpGet, ermöglicht es, zusätzlich Header-Informationen
	 * mitzugeben.
	 *
	 * @param client  the client
	 * @param uri     Die URL auf welche der GET-Request abgesetzt werden soll.
	 ...
	 * @throws IOException the io exception
	 */
	public static CloseableHttpResponse doHttpGet(CloseableHttpClient client, String uri, List<NameValuePair> headers)
			throws IOException {
		HttpRequestBase httpMethod = new HttpGet(uri);
		for (NameValuePair header : headers) {
			httpMethod.addHeader(header.getName(), header.getValue());
	...
	/**
	 * Setzt einen Http-Post Request gegen eine bestimmte URI mit bestimmten
	 * Parametern ab.
	 *
	 * @param uri        Die URI gegen welche der Request abgesetzt werden soll.
	...
	/**
	 * Overload von doHttpPut, ermöglicht es, zusätzlich Header-Informationen
	 * mitzugeben.
	 *
	 * @param client  the client
	 * @param uri     Die URL auf welche der PUT-Request abgesetzt werden soll.
	 ...
	 * @throws IOException the io exception
	 */
	public static CloseableHttpResponse doHttpPut(CloseableHttpClient client, String uri, List<NameValuePair> headers)
			throws IOException {
		HttpRequestBase httpMethod = new HttpPut(uri);
		for (NameValuePair header : headers) {
			httpMethod.addHeader(header.getName(), header.getValue());
	...
	/**
	 * Overload von HttpDelete, ermöglicht es, zusätzlich Header-Informationen
	 * mitzugeben.
	 *
	 * @param client  the client
	 ...
	 * @throws IOException the io exception
	 */
	public static CloseableHttpResponse doHttpDelete(CloseableHttpClient client, String uri,
			List<NameValuePair> headers) throws IOException {
	...
	public static CloseableHttpResponse callRestMethod(CloseableHttpClient client, RestMethod methodType, String uri,
			ContentType contentType, String content, List<NameValuePair> headers) throws IOException {
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static CloseableHttpResponse callRestMethod(final CloseableHttpClient httpclient,
			final HttpRequestBase method) throws IOException {
	 ...
	 * @throws IOException Signals that an I/O exception has occurred.
	 */
	public static int getResponseCode(final CloseableHttpClient httpclient, final HttpRequestBase method)
			throws IOException {
